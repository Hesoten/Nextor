...
| `DRV_DIRECT0-4` (4142h-414Eh) | `DIRECT_0-4` (4134h-4140h) | Same feature, new addresses; the kernel-side entry points moved from 7450h-745Ch to 7850h-785Ch |
| `DRV_CONFIG` (4151h) | Gone | Configuration index 1 is covered by driver query 5 (default: 4 devices); index 2 is meaningless without logical units |
...
* `DO_DRVQ_GET_MAX_DEVICE` replaces the drive-count part of `DRV_CONFIG`; return the highest device number your driver handles, or `RESULT_NOT_IMPLEMENTED` to get the default of 4 (see _[4.5.5. Driver query 5: Get maximum supported device number](Nextor%203.0%20Driver%20Development%20Guide.md#455-driver-query-5-get-maximum-supported-device-number)_).
...
* `DRV_CONFIG`: configuration index 1 (number of drives) became driver query 5, already handled in step 3.5. Configuration index 2 (default drive mapping per logical unit) is meaningless now that logical units are gone; the related boot-mapping tweaks are covered by the per-device "don't use for automapping" flag in the device parameters (bit 3 of the flags byte, see _[4.6.2. Device query 2: Get device parameters](Nextor%203.0%20Driver%20Development%20Guide.md#462-device-query-2-get-device-parameters)_).
...
In the Nextor 2 era, drivers typically lived inside a fork of the Nextor kernel repository and were built by the kernel's own Makefile. In Nextor 3 drivers live in standalone repositories and build against two artifacts:
